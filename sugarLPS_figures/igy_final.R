...
df1 <- df1 %>% 
  mutate(timenum = str_replace(time, "0423igy", "2")) %>%
  mutate(timenum = str_replace(timenum, "0428igy", "5")) %>%
  mutate(timenum = str_replace(timenum, "0430igy", "7")) %>%
  mutate(timenum = str_replace(timenum, "0504igy", "11")) %>%
  mutate(timenum = str_replace(timenum, "0511igy", "17")) %>%
  mutate(timenum = str_replace(timenum, "0525igy", "30")) %>%
  mutate(timenum = str_replace(timenum, "0528igy", "33")) %>%
  mutate(timenum = str_replace(timenum, "0530igy", "35")) %>%
  mutate(timenum = str_replace(timenum, "0603igy", "39")) %>%
  mutate(timenum = str_replace(timenum, "0610igy", "46")) %>%
  mutate(timenum = str_replace(timenum, "0624igy", "60")) 
df1 <- df1 %>% 
...
p1 <- ggplot(data = df1, aes(x = timenum, y = mean, group = Treatments)) +
  ...
  annotate("text", x = c(10,38), y=.2, colour= "red", label = c("1st\nChallenge", "2nd\nChallenge"))


p1


pdf('Fig4_igy.pdf', width = 6, height = 5)
p1
dev.off()
